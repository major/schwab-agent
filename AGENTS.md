...
- **Tier 1** (no auth, CI-safe): help text for all 77 commands/subcommands, symbol build/parse, all 18 order build sub-types with full permutations, flag alias tests, shorthand/alias error cases (183 tests). Runs in CI and locally via `make smoke-ci`.
