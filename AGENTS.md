...
internal/
  ...
  orderbuilder/         Order construction/validation (equity, option, bracket, OCO, jade lizard) + OCC symbol build/parse
...
- **Tier 1** (no auth, CI-safe): help text for all commands/subcommands, symbol build/parse, all 34 order build sub-types (including 15 porcelain commands such as `sell-covered-call` and `short-iron-condor`) with full permutations, flag alias tests, shorthand/alias error cases, porcelain direction-flag rejection tests (300 tests). Runs in CI and locally via `make smoke-ci`.
...
10. **Quote option mode**: `quote get` defaults to compact quote-only fields and accepts structured option flags (`--underlying`, `--expiration`, `--strike`, `--call`/`--put`) as an alternative to positional symbol args. Uses `cobra.ArbitraryArgs` and detects option mode via `cmd.Flags().Changed()`.
...
13. **Porcelain command pattern**: Porcelain commands (e.g., `long-call`, `put-credit-spread`, `short-iron-condor`, `jade-lizard`) hardcode direction, put/call, and open/close so agents supply only underlying, expiration, strikes, quantity, and price. Each porcelain family uses a factory closure (`parseSingleLegFactory`, `parsePorcelainVerticalFactory`, etc.) that captures the hardcoded values and returns a parse function compatible with the generic `makeCobraBuildOrderCommand`/`makeCobraPlaceOrderCommand`/`makeCobraPreviewOrderCommand` factories unless the command needs a preflight hook. Vertical spreads use `--high-strike`/`--low-strike` (factual market position) instead of `--long-strike`/`--short-strike` (directional), with the command name determining which strike is bought vs sold. `sell-covered-call` is intentionally custom for preview/place: it rechecks long shares before preview and before final saved-preview placement so agents cannot accidentally submit an uncovered short call through the digest workflow. Generic commands remain for power users and closing positions.
14. **Analyze scan output pattern**: `analyze` batches quote retrieval through `GetQuotes()` for all requested symbols, then fetches TA dashboards concurrently with a small fixed limit. Normal output preserves the full quote plus TA dashboard shape. `--latest-only` omits duplicate dashboard `values` rows only from analyze output, and `--compact` emits token-efficient quote, technical, and signal maps for watchlist scans.
