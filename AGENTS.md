...
- **Tier 1** (no auth, CI-safe): help text for all 120 commands/subcommands, symbol build/parse, all 32 order build sub-types (including 13 porcelain) with full permutations, flag alias tests, shorthand/alias error cases, porcelain direction-flag rejection tests (278 tests). Runs in CI and locally via `make smoke-ci`.
