...
- **Tier 1** (no auth, CI-safe): help text for all commands/subcommands, symbol build/parse, all 32 order build sub-types (including 13 porcelain) with full permutations, flag alias tests, shorthand/alias error cases, porcelain direction-flag rejection tests (285 tests). Runs in CI and locally via `make smoke-ci`.
...
13. **Porcelain command pattern**: Porcelain commands (e.g., `long-call`, `put-credit-spread`, `jade-lizard`) hardcode direction, put/call, and open/close so agents supply only underlying, expiration, strikes, quantity, and price. Each porcelain family uses a factory closure (`parseSingleLegFactory`, `parsePorcelainVerticalFactory`, etc.) that captures the hardcoded values and returns a parse function compatible with the generic `makeCobraBuildOrderCommand`/`makeCobraPlaceOrderCommand`/`makeCobraPreviewOrderCommand` factories. Vertical spreads use `--high-strike`/`--low-strike` (factual market position) instead of `--long-strike`/`--short-strike` (directional), with the command name determining which strike is bought vs sold. Generic commands remain for power users and closing positions.
