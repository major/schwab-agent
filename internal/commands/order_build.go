...
or to order preview --spec - to check estimated commissions.`
  ...
  schwab-agent order build equity --symbol AAPL --action BUY --quantity 10 --type LIMIT --price 200 | schwab-agent order place --spec -`
	...
	optionBuild := makeCobraBuildOrderCommand(
		w,
		"option",
...
suitable for piping to order place or order preview.`
  ...
  schwab-agent order build option --underlying AAPL --expiration 2025-06-20 --strike 190 --put --action SELL_TO_OPEN --quantity 1`
	...
	bracketBuild := makeCobraBuildOrderCommand(
		w,
		"bracket",
		"Build a bracket order request",
...
auto-inverted from the entry action.`
  ...
  schwab-agent order build bracket --symbol AAPL --action BUY --quantity 10 --type LIMIT --price 180 --stop-loss 170`
	...
	ocoBuild := makeCobraBuildOrderCommand(
		w,
		"oco",
		"Build a one-cancels-other order request for an existing position",
...
NET_DEBIT or NET_CREDIT is auto-determined from the strike relationship.`
  ...
  schwab-agent order build vertical --underlying F --expiration 2026-06-18 --long-strike 14 --short-strike 12 --put --open --quantity 1 --price 0.50`
	...
	ironCondorBuild := makeCobraBuildOrderCommand(
		w,
		"iron-condor",
...
generates a NET_CREDIT.`
	...
	straddleBuild := makeCobraBuildOrderCommand(
		w,
		"straddle",
...
or --sell for short strangles (expecting low volatility).`
  ...
  schwab-agent order build strangle --underlying F --expiration 2026-06-18 --call-strike 14 --put-strike 10 --sell --open --quantity 1 --price 0.50`
	...
	coveredCallBuild := makeCobraBuildOrderCommand(
		w,
		"covered-call",
		"Build a covered call order request (buy shares + sell call)",
		func() *coveredCallBuildOpts { return &coveredCallBuildOpts{} },
		func(cmd *cobra.Command, opts *coveredCallBuildOpts) { defineAndConstrain(cmd, opts) },
		parseCoveredCallParams,
		orderbuilder.ValidateCoveredCallOrder,
		orderbuilder.BuildCoveredCallOrder,
	)
...
already own, use order place option --action SELL_TO_OPEN instead.`
	...
	collarBuild := makeCobraBuildOrderCommand(
		w,
		"collar",
...
capping upside. Quantity 1 means 100 shares plus 1 put and 1 call contract.`
	...
	calendarBuild := makeCobraBuildOrderCommand(
		...
		orderbuilder.BuildCalendarOrder,
	)
...
contract. Profits from time decay differential between the two legs.`
	...
	diagonalBuild := makeCobraBuildOrderCommand(
		w,
		"diagonal",
		...
		orderbuilder.BuildDiagonalOrder,
	)
...
contract at a different strike. Combines elements of vertical and calendar spreads.`
	...
	butterflyBuild := makeCobraBuildOrderCommand(
		w,
		"butterfly",
		...
		orderbuilder.BuildButterflyOrder,
	)
...
butterfly with long wings and two short body contracts.`
	...
	condorBuild := makeCobraBuildOrderCommand(
		w,
		"condor",
...
opens a long condor; selling opens the inverse short condor.`
	...
	verticalRollBuild := makeCobraBuildOrderCommand(
		...
		orderbuilder.BuildVerticalRollOrder,
	)
...
pricing direction.`
	...
	backRatioBuild := makeCobraBuildOrderCommand(
		w,
		"back-ratio",
		"Build a back-ratio spread order request",
		func() *backRatioBuildOpts { return &backRatioBuildOpts{} },
		func(cmd *cobra.Command, opts *backRatioBuildOpts) {
			defineAndConstrain(
				cmd,
				opts,
				[]string{"call", "put"},
		...
		parseBackRatioParams,
		orderbuilder.ValidateBackRatioOrder,
		orderbuilder.BuildBackRatioOrder,
	)
...
short contract. Use --debit or --credit because back-ratios can price either way.`
	...
	doubleDiagonalBuild := makeCobraBuildOrderCommand(
		w,
		"double-diagonal",
		...
		orderbuilder.BuildDoubleDiagonalOrder,
	)
